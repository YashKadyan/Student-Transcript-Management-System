...
<body>
        ...
        <ul class="navbar-nav">
          ...
          <li class="nav-item">
            ...
            <div class="dropdown-menu dropdown-menu" style="margin-left:34%;margin-top:-1%;">
                <span class="decorate"><a class="dropdown-item" href="addfaculty.php"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Add Faculty</a></span>
              </div>
          </li>
          <li class="nav-item">
              <a class="nav-link dropdown-toggle" href="#" id="navbardrop"  data-toggle="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu" style="margin-left:39.6%;margin-top:-1%;">
    ...
    <div class="form-group row">
        <label for="name" class="col-sm-2 col-form-label">Faculty Name</label>
        <div class="col-sm-5">
          ...
          <div class="invalid-tooltip">Please enter the Faculty Name!</div>
        </div>
    </div>
    <div class="form-group row">
        <label for="profile_pic" class="col-sm-2 col-form-label">Faculty Profile<br>Photo</label>
        <div class="col-sm-5">
          ...
          <div class="invalid-tooltip">Please upload the Faculty Image!</div>
        </div>       
    </div>
    <div class="form-group row ">
      ...
      <label for="add" class="col-sm-2 col-form-label" ></label>
        <div class="col-sm-3">
          <button type="submit" name="add" class="btn btn-primary">Add Faculty Details</button>
        </div>
      </div>
      <?php
        ...
        if(isset($_POST['add']) && $_FILES['filepath']["error"]==0)
        {
            ...
            if($new_row[0]==0)
            {
                ...
                {?>
                    ...
                    <strong>Success!</strong> Faculty Details added successfully!
                ...
                {?>
                    ...
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>Error!</strong>
                    Faculty Details could not be inserted!<?php echo mysqli_error($connection);?>
                    </div> 
                    <?php
                }
            }
            ...
            {?>
                ...
                <button type="button" class="close" data-dismiss="alert">&times;</button>
                <strong>Error!</strong> Faculty with the given name already exists!
                </div>
                <?php
            }
            $new_sql="SELECT COUNT(username) from log_in where username='".$uemail."' and role_id=2";
            $new_result=mysqli_query($connection, $new_sql);
            $new_row=mysqli_fetch_row($new_result);
            if($new_row[0]==0)
            {
                $nms = explode(' ',$uname);
                $pass = $nms[0]."@123";
                $new_sql="INSERT INTO log_in(username,pass,role_id,user_id) VALUES('".$uemail."','".MD5($pass)."',2,$UID)";
                if(mysqli_query($connection, $new_sql))
                {?>
                    <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>Success!</strong> Faculty Login Details given successfully!
                    </div>
                  <?php
                }
                else
                {?>
                    ...
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>Error!</strong>
                    Faculty Login Details could not be given successfully!<?php echo mysqli_error($connection);?>
                    </div> 
                  <?php
                }
            }
            ...
            {?>
                <div class="alert alert-warning alert-dismissable">
                <button type="button" class="close" data-dismiss="alert">&times;</button>
                <strong>Error!</strong> Faculty Login already exists!
                </div>
                <?php
            }
            
            $new_sql="SELECT COUNT(user_id) from faculty WHERE dept_id='".$deptid."' and college_id='".$clgid."' and  user_id in(SELECT user_id from user WHERE u_name='".$uname."' and email='".$uemail."')";
            $new_result=mysqli_query($connection, $new_sql);
            $new_row=mysqli_fetch_row($new_result);
            if($new_row[0]==0)
            {
                $new_sql="INSERT INTO faculty(user_id,dept_id,college_id) VALUES($UID,$deptid ,$clgid)";
                if(mysqli_query($connection, $new_sql))
                {?>
                    <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>Success!</strong> Faculty Added into College successfully!
                    </div>
                  <?php
                }
                else
                {?>
                    ...
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>Error!</strong>
                    Faculty could not be added successfully!<?php echo mysqli_error($connection);?>
                    </div> 
                  <?php
                }
            }
            ...
            {?>
                <div class="alert alert-warning alert-dismissable">
                <button type="button" class="close" data-dismiss="alert">&times;</button>
                <strong>Error!</strong> Faculty with the given name has already taken admission in college!
                </div>
                <?php
            }
        }
